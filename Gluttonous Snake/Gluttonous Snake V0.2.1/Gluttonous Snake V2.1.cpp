...
相比2.0，我把关键帧判定抽离玩家类，并将其分化成逻辑层和渲染层，，简单尝试把逻辑计算和渲染分离
并且，关于刷新率问题暂时设定为100帧（与键盘监听同频）但是蛇的移动（刷新率）固定为5，10，20Hz
刷新方式暂时为需要刷新就调用渲染函数
(并且由于贪吃蛇移动帧率不能太高，还是会出现异步刷新)
（不如改成一起加速？）
（或者用统一计数器，松开键盘后自动对其刷新时刻？）
同时，关键帧判定变成持续判定，哪怕键盘没有按下都会进行，我不清楚这样的方式是否正确
同时，因为又一次改变架构，很多地方代码重复，调用繁琐，并且有一定的bug（比如抢同一个格子、果子
会显示异常，在蛇两次刷新之间快速按下左/右方向键在按会向前键，会判定为吃掉旁边的果子并正确增长
蛇身和刷新果子）
*/
...
public:
    ...
    void setV(long long item) {
        if (LOW_DOWN_TIME <= item && item <= 4 * LOW_DOWN_TIME) {
            V = SNAKE_UPDATE / 2;//10Hz/10
        }
        else if (4 * LOW_DOWN_TIME <= item) {
            V = SNAKE_UPDATE / 4;//20Hz/5
        }
        else {
            V = SNAKE_UPDATE;
        }
...
public:
    ...
    void updataMove(int x, int y, int dir, int id) {
        ...
        int nx = x + dx[dir];
        int ny = y + dy[dir];
        int px = x + NextMove[id][0];
        int py = y + NextMove[id][1];
...
public:
    void renderGame() {
        ...
        //打印果子
        for (int i = 0; i < 3; i++) {
            Print(g_GameLogic.getDotData(i, 2), g_GameLogic.getDotData(i, 3), 2);
        }
...
void ATGame() {
    ...
    if (g_Player1.getFrame() == 0 || g_Player2.getFrame() == 0) {
        g_GameRander.renderGame();
    }
}
        ...
        if (msg.message == WM_LBUTTONUP) {
            ...
            if (msg.x >= Rarea.left && msg.x <= Rarea.right && msg.y >= Rarea.top && msg.y <= Rarea.bottom) {
                ...
                if (g_GameSta.getStaDepth() == 2) {
                    ...
                    g_GameSta.pop();
                    g_GameTime.resume();
                    //g_GameLogic.BeforeRander1(g_GameSta.getStaID());
                    //g_GameLogic.BeforeRander2(g_GameSta.getStaID());
                    g_GameRander.renderGame();
                }
                else if (g_GameSta.getStaID() == STA_OVER || g_GameSta.getStaID() == STA_ACCOUNT) {
                    //重开（我应该使用StaID？）
