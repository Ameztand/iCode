...
相比3.0，分离了移动请求和更新数据，并且可以提取一个子函数出来（暂时没用提取）
加入死亡动画，可以用ESC退出
*/
...
void InitLobby();   void Lobby();
void InitSelect();  void Select();

void InitPause();   void Pause();
void InitOver();    void Over();
void InitAccount(); void Account();

void InitDeathAnim(); void DeathAnim();

void InitPuGameData(); void PuGame();
void InitATGameData(); void ATGame();
void InitEXGameData(); void EXGame();
...
public:
    ...
    //判断移动请求
    void judgeMoveRequest(Player& player) {
        //获取下一格坐标
    ...
    int getLoser() {
        //头撞头判定
        if (g_GameSta.getStaID() == STA_AT_GAME || g_GameSta.getStaID() == STA_EX_GAME) {
...
public:
    void renderOver(const Logic& logic) {
        ...
        if (Frame_ > SNAKE_UPDATE) {
            ...
            for (const Player& player : players) {// 遍历所有玩家进行绘制
                ...
                if (Death_[playerID] == true && RenderCnt_ % 2 == 0) {
                ...
                for (int i = temp.size(); i > 0; i--) {
                    ...
                    x = pos.x;
                    y = pos.y;
                    Print(x, y, (i == 1 ? 4 : (playerID == g_GameLogic.getMirror() ? 1 : 3)));
                    temp.pop();
                }
                //playerID++;
            }
    ...
    void renderGame(const Logic& logic) {
        ...
        for (const Player& player : players) {// 遍历所有玩家进行绘制
            ...
            for (int i = temp.size(); i > 0; i--) {
                ...
                x = pos.x;
                y = pos.y;
                Print(x, y, (i == 1 ? 4 : (playerID == g_GameLogic.getMirror() ? 1 : 3)));//第563行
                temp.pop();
            }
            playerID++;
        }
        ...
        if (g_GameSta.getStaID() == STA_PU_GAME) {
            swprintf(buffer, 256, L"当前速度：%d / %lld                                                                      ",
                1000 / g_GameLogic.getPlayer(0).getV(), g_BackTime.getSum());
            outtextxy(800, 340, buffer);
        }
        else if (g_GameSta.getStaID() == STA_AT_GAME) {
            swprintf(buffer, 256, L"当前速度：%d / %d / %lld / %lld                                                                      ",
                1000 / g_GameLogic.getPlayer(0).getV(), 1000 / g_GameLogic.getPlayer(1).getV(), g_BackTime.getSum(), g_EnterTime.getSum());
            outtextxy(800, 340, buffer);
        }
        else if (g_GameSta.getStaID() == STA_EX_GAME) {
            swprintf(buffer, 256, L"当前速度：%d / %d / %d / %d                                                                      ",
                1000 / g_GameLogic.getPlayer(0).getV(), 1000 / g_GameLogic.getPlayer(1).getV(), g_GameLogic.getPlayer(0).getE(), g_GameLogic.getPlayer(1).getE());
            outtextxy(800, 340, buffer);
        }
        ...
        //打印长度提示
        if (g_GameSta.getStaID() == STA_PU_GAME) {
            swprintf(buffer, 256, L"当前长度：%d                                          ", (int)g_GameLogic.getPlayer(0).sizeQ());
            outtextxy(800, 380, buffer);
        }
        else if (g_GameSta.getStaID() == STA_AT_GAME || g_GameSta.getStaID() == STA_EX_GAME) {
            swprintf(buffer, 256, L"当前长度：%d / %d                                          ", (int)g_GameLogic.getPlayer(0).sizeQ(), (int)g_GameLogic.getPlayer(1).sizeQ());
            outtextxy(800, 380, buffer);
        }
        ...
        // 5. 空格键（中心方块）
        if (g_GameMSG.getKeySta(VK_SPACE))
            setfillcolor(pressedGreen);
        else
            setfillcolor(normal);
        ...
        // 7. 左方向键
        if (g_GameMSG.getKeySta(VK_LEFT))
            setfillcolor(pressedBlue);
        else
            setfillcolor(normal);
        ...
        // 8. 下方向键
        if (g_GameMSG.getKeySta(VK_DOWN))
            setfillcolor(pressedBlue);
        else
            setfillcolor(normal);
        ...
        // 9. 右方向键
        if (g_GameMSG.getKeySta(VK_RIGHT))
            setfillcolor(pressedBlue);
        else
            setfillcolor(normal);
        ...
        // 10. 回车键（中心方块）
        if (g_GameMSG.getKeySta(VK_RETURN))
            setfillcolor(pressedBlue);
        else
            setfillcolor(normal);
...
void Lobby() {
    //大厅检测
    if (g_GameMSG.getMouse()) {
        POINT pt = g_GameMSG.getMousePos();
    ...
    //ESC
    if (g_GameMSG.getKeyDown(VK_ESCAPE)) {
        printf("键盘按下，Sta=%d\n", g_GameSta.getStaID());
        g_GameSta.pop();
        g_GameRenderer.UpdateMenu();
    }
...
void Select() {
    ...
    if (g_GameMSG.getMouse()) {
        ...
        if (pt.x >= Larea.left && pt.x <= Larea.right && pt.y >= Larea.top && pt.y <= Larea.bottom) {
            //左
            //极限模式
            g_GameSta.push(InitEXGameData, STA_EX_GAME);
            printf("左\n");
        }
...
void Pause() {
    ...
    if (g_GameMSG.getMouse()) {
        ...
        if (pt.x >= Rarea.left && pt.x <= Rarea.right && pt.y >= Rarea.top && pt.y <= Rarea.bottom) {
            ...
            g_GameSta.pop();
            g_GameTime.resume();
            g_GameRenderer.renderGame(g_GameLogic);
            printf("右\n");
        }
    }
...
void InitDeathAnim() {
    bool Death[2] = { false };
    const std::vector<Player>& players = g_GameLogic.getPlayerList();
    int playerID = 0;
    for (const Player& player : players) {// 遍历所有玩家进行绘制
        Death[playerID] = !player.getLive();
        playerID++;
    }
    g_GameRenderer.initRenderOver(Death);
    g_GameSta.pop();
    g_GameSta.push(DeathAnim, STA_DEATH);
}

void DeathAnim() {
    if (!g_GameMSG.getKeyDown(VK_ESCAPE) && g_GameRenderer.getRenderCnt() > 0) {
        g_GameRenderer.renderOver(g_GameLogic);
    }
    else {
        g_GameSta.pop();
        if (g_GameSta.getStaID() == STA_PU_GAME || g_GameSta.getStaID() == STA_EX_GAME) {
...
void InitPuGameData() {
    ...
    g_GameLogic.clearMapData();
    g_GameLogic.clearPlayerList();
    g_GameLogic.initMirror();

    //队列初始化
    g_GameLogic.addPlayer(0, 0, 0);
...
void PuGame() {
    ...
    //键盘监听
    if (g_GameMSG.getKeyDown(VK_ESCAPE)) {//ESC
        printf("键盘按下，Sta=%d\n", g_GameSta.getStaID());
        g_GameSta.push(InitPause, STA_PAUSE);
        g_BackTime.clear();
        g_GameTime.pause();
    }
    if (g_GameMSG.getKeyDown(VK_SPACE)) {//空格
        if (g_BackTime.getSum() == 0) {
            g_BackTime.start();
        }
    }
    if (g_GameMSG.getKeyUp(VK_SPACE)) {//空格
        g_BackTime.clear(); // 清零
    }
    if (g_GameMSG.getKeyDown(VK_UP) || g_GameMSG.getKeyDown('W')) {
        g_GameLogic.getPlayer(0).up();
    }
    if (g_GameMSG.getKeyDown(VK_RIGHT) || g_GameMSG.getKeyDown('D')) {
        g_GameLogic.getPlayer(0).right();
    }
    if (g_GameMSG.getKeyDown(VK_DOWN) || g_GameMSG.getKeyDown('S')) {
        g_GameLogic.getPlayer(0).down();
    }
    if (g_GameMSG.getKeyDown(VK_LEFT) || g_GameMSG.getKeyDown('A')) {
        g_GameLogic.getPlayer(0).left();
    }
    ...
    bool p1 = (g_GameLogic.getPlayer(0).getV() <= g_GameLogic.getPlayer(0).getFrame());

    if (p1) {
        //清空帧数计数
        g_GameLogic.getPlayer(0).clearFrame();
        g_GameLogic.judgeMoveRequest(g_GameLogic.getPlayer(0));
    }

    //渲染
    if (g_GameLogic.getLoser() == 0) {
        if (p1)g_GameLogic.updataData(g_GameLogic.getPlayer(0));
        g_GameRenderer.renderGame(g_GameLogic);
    }
    else {
        g_GameSta.push(InitDeathAnim, STA_DEATH);
    }
}
...
void ATGame() {
    ...
    if (g_GameMSG.getKeyDown(VK_RETURN)) {//回车
        if (g_EnterTime.getSum() == 0) {
            g_EnterTime.start();
        }
    }
    if (g_GameMSG.getKeyUp(VK_RETURN)) {//回车
        g_EnterTime.clear(); // 清零
        g_GameLogic.getPlayer(1).setFrame(g_GameLogic.getPlayer(0).getFrame());
    }
    // 玩家0：WASD
    if (g_GameMSG.getKeyDown('W')) {
        g_GameLogic.getPlayer(0).up();
    }
    if (g_GameMSG.getKeyDown('D')) {
        g_GameLogic.getPlayer(0).right();
    }
    if (g_GameMSG.getKeyDown('S')) {
        g_GameLogic.getPlayer(0).down();
    }
    if (g_GameMSG.getKeyDown('A')) {
        g_GameLogic.getPlayer(0).left();
    }
    // 玩家1：方向键
    if (g_GameMSG.getKeyDown(VK_UP)) {
        g_GameLogic.getPlayer(1).up();
    }
    if (g_GameMSG.getKeyDown(VK_RIGHT)) {
        g_GameLogic.getPlayer(1).right();
    }
    if (g_GameMSG.getKeyDown(VK_DOWN)) {
        g_GameLogic.getPlayer(1).down();
    }
    if (g_GameMSG.getKeyDown(VK_LEFT)) {
        g_GameLogic.getPlayer(1).left();
    }
    ...
    if (g_GameLogic.getPlayer(0).getV() <= g_GameLogic.getPlayer(0).getFrame()) toMove.push_back(0);
    if (g_GameLogic.getPlayer(1).getV() <= g_GameLogic.getPlayer(1).getFrame()) toMove.push_back(1);

    for (int i : toMove) {
        //清空帧数计数
        g_GameLogic.getPlayer(i).clearFrame();
        g_GameLogic.judgeMoveRequest(g_GameLogic.getPlayer(i));
    }
    ...
    for (int i : toMove) {
        //清空帧数计数
        g_GameLogic.updataData(g_GameLogic.getPlayer(i));
    }
...
void InitEXGameData() {
    ...
    //队列初始化
    g_GameLogic.addPlayer(5, -5, 0);
    g_GameLogic.addPlayer(-5, 5, 2);

    //地图初始化和果子初始化
    g_GameLogic.initMapData(STA_EX_GAME);
    ...
    g_GameSta.pop();
    g_GameSta.push(EXGame, STA_EX_GAME);
}
...
void EXGame() {
    ...
    bool mirror = g_GameLogic.getMirror(); // 注意：原代码中用 mirror 变量，但 Logic 中 mirror 是私有，需要提供 getMirror() 已存在
    int mainIdx = mirror ? 1 : 0;
    int subIdx = 1 - mainIdx;

    // 上：主玩家 up，副玩家 down
    if (g_GameMSG.getKeyDown(VK_UP) || g_GameMSG.getKeyDown('W')) {
        g_GameLogic.getPlayer(mainIdx).up();
        g_GameLogic.getPlayer(subIdx).down();
    }
    // 右：主玩家 right，副玩家 left
    if (g_GameMSG.getKeyDown(VK_RIGHT) || g_GameMSG.getKeyDown('D')) {
        g_GameLogic.getPlayer(mainIdx).right();
        g_GameLogic.getPlayer(subIdx).left();
    }
    // 下：主玩家 down，副玩家 up
    if (g_GameMSG.getKeyDown(VK_DOWN) || g_GameMSG.getKeyDown('S')) {
        g_GameLogic.getPlayer(mainIdx).down();
        g_GameLogic.getPlayer(subIdx).up();
    }
    // 左：主玩家 left，副玩家 right
    if (g_GameMSG.getKeyDown(VK_LEFT) || g_GameMSG.getKeyDown('A')) {
        g_GameLogic.getPlayer(mainIdx).left();
        g_GameLogic.getPlayer(subIdx).right();
    }
